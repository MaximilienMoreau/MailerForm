...
function Cell({ value, highlight }: { value: CellValue; highlight?: boolean }) {
  if (value === 'best') {
    return (
      <td className={`px-4 py-3 text-center ${highlight ? 'bg-brand-500/5' : ''}`}>
  ...
  if (value === 'partial') {
    return (
      <td className={`px-4 py-3 text-center ${highlight ? 'bg-brand-500/5' : ''}`}>
        <Minus size={15} className="text-amber-400 mx-auto" />
      </td>
    )
  }
  return (
    <td className={`px-4 py-3 text-center ${highlight ? 'bg-brand-500/5' : ''}`}>
      <div className="w-3 h-0.5 bg-gray-700 mx-auto rounded" />
    </td>
  )
}
  ...
  return (
    <section ref={ref} className="py-24 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {rows.map((row) => (
                <tr key={row.label} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <Cell value={row.resend as CellValue} />
                </tr>
              ))}
            </tbody>
          </table>
        ...
        >
          <span className="flex items-center gap-1.5"><Star size={11} className="text-brand-400 fill-brand-400" /> Best-in-class</span>
          <span className="flex items-center gap-1.5"><Check size={11} className="text-emerald-400" /> Supported</span>
          <span className="flex items-center gap-1.5"><Minus size={11} className="text-amber-400" /> Partial</span>
          <span className="flex items-center gap-1.5"><div className="w-3 h-0.5 bg-gray-700 rounded" /> Not available</span>
        </motion.div>
      </div>
    </section>
  )
